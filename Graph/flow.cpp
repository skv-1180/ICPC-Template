...
// MCMF F(n); //0-idx 
// F.add_edge(u,v,cap,cost); //if edge is undirected-> make it false
// int s = n, t = s + 1;
// F.add_edge(s, 0, k, 0); 
// F.add_edge(n-1, t, k, 0);
// pi ans = F.solve(s, t); // returns {maxflow, mincost}
/*
If while adding edges, we add id also then
F.flow_through[id] -> gives amount of flow through edge(id) 
*/
